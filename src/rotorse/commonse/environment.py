...
class WindBase(Component):
    def __init__(self):
        ...
        # variables
        self.add_param('Uref', units='m/s', desc='reference wind speed (usually at hub height)')
        self.add_param('zref', units='m', desc='corresponding reference height')
        self.add_param('z', units='m', desc='heights where wind speed should be computed')

        # parameters
        self.add_param('z0', val=0.0, units='m', desc='bottom of wind profile (height of ground/sea)')

        # out
        self.add_output('U', units='m/s', desc='magnitude of wind speed at each z location')
        self.add_output('beta', units='deg', desc='corresponding wind angles relative to inertial coordinate system')
...
class WaveBase(Component):
    def __init__(self):
        super(WaveBase).__init__()
        """base component for wave speed/direction"""

        # variables
        self.add_param('z', units='m', desc='heights where wave speed should be computed')
        self.add_param('z_surface', units='m', desc='vertical location of water surface')
        self.add_param('z_floor', val=0.0, units='m', desc='vertical location of sea floor')

        # out
        self.add_output('U', units='m/s', desc='magnitude of wave speed at each z location')
        self.add_output('A', units='m/s**2', desc='magnitude of wave acceleration at each z location')
        self.add_output('beta', units='deg', desc='corresponding wave angles relative to inertial coordinate system')
        self.add_output('U0', units='m/s', desc='magnitude of wave speed at z=MSL')
        self.add_output('A0', units='m/s**2', desc='magnitude of wave acceleration at z=MSL')
        self.add_output('beta0', units='deg', desc='corresponding wave angles relative to inertial coordinate system at z=MSL')
...
class SoilBase(Component):
    def __init__(self):
        super(SoilBase).__init__()
        """base component for soil stiffness"""

        # out
        self.add_output('k', units='N/m', required=True, desc='spring stiffness. rigid directions should use \
...
class LinearWaves(WaveBase):
    def __init__(self):
        super(LinearWaves).__init__()
        """linear (Airy) wave theory"""

        # variables
        self.add_param('Uc', units='m/s', desc='mean current speed')

        # parameters
        self.add_param('hs', units='m', desc='significant wave height (crest-to-trough)')
        self.add_param('T', units='s', desc='period of waves')
...
class TowerSoilK(SoilBase):
    def __init__(self):
        super(TowerSoilK).__init__()
        """Passthrough of Soil-Structure-INteraction equivalent spring constants used to bypass TowerSoil."""

        # variable
        self.add_param('kin',  desc='spring stiffness. rigid directions should use \
            ``float(''inf'')``. order: (x, theta_x, y, theta_y, z, theta_z)')

        self.add_param('rigid', dtype=np.bool, desc='directions that should be considered infinitely rigid\
