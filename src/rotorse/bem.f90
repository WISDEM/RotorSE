...
!
!  Differentiation of inductionfactors in forward (tangent) mode:
!   variations   of useful results: fzero
!   with respect to varying inputs: cd cl
!   RW status of diff variables: fzero:out cd:in cl:in
SUBROUTINE COEFFICIENTS_DV(r, chord, rhub, rtip, phi, cl, cld, cd, &
& cdd, b, vx, vy, usecd, hubloss, tiploss, wakerotation, fzero, fzerod, &
& a, ap, nbdirs)
!  USE DIFFSIZES
!  Hint: nbdirsmax should be the maximum number of differentiation directions
  IMPLICIT NONE
...
! in
  REAL(dp), INTENT(IN) :: r, chord, rhub, rtip, phi, cl, cd
  REAL(dp), DIMENSION(nbdirs), INTENT(IN) :: cld, cdd
...
! out
  REAL(dp), INTENT(OUT) :: fzero, a, ap
  REAL(dp), DIMENSION(nbdirs), INTENT(OUT) :: fzerod
! local
  REAL(dp) :: pi, sigma_p, sphi, cphi, lambda_r
  REAL(dp) :: factortip, ftip, factorhub, fhub
  REAL(dp) :: k, kp, cn, ct, f
  REAL(dp), DIMENSION(nbdirs) :: kd, kpd, cnd, ctd
  REAL(dp) :: g1, g2, g3
  REAL(dp), DIMENSION(nbdirs) :: g1d, g2d, g3d
  INTRINSIC KIND
  INTRINSIC SIN
...
! resolve into normal and tangential forces
  ...
  ELSE
    DO nd=1,nbdirs
      cnd(nd) = cphi*cld(nd) + sphi*cdd(nd)
      ctd(nd) = sphi*cld(nd) - cphi*cdd(nd)
    END DO
    cn = cl*cphi + cd*sphi
    ct = cl*sphi - cd*cphi
    print *, cnd
...
! Prandtl's tip and hub loss factor
  ...
  DO nd=1,nbdirs
! bem parameters
    kd(nd) = sigma_p*cnd(nd)/4.0_dp/f/sphi/sphi
    kpd(nd) = sigma_p*ctd(nd)/4.0_dp/f/sphi/cphi
  END DO
  k = sigma_p*cn/4.0_dp/f/sphi/sphi
  kp = sigma_p*ct/4.0_dp/f/sphi/cphi
! compute axial induction factor
...
! momentum state
    ...
    ELSE
      DO nd=1,nbdirs
! Glauert(Buhl) correction
        g1d(nd) = 2.0_dp*f*kd(nd)
        g2d(nd) = 2.0_dp*f*kd(nd)
        g3d(nd) = 2.0_dp*f*kd(nd)
      END DO
      g1 = 2.0_dp*f*k - (10.0_dp/9-f)
      g2 = 2.0_dp*f*k - (4.0_dp/3-f)*f
      g3 = 2.0_dp*f*k - (25.0_dp/9-2*f)
...
! error function
  ...
  IF (phi .GT. 0) THEN
    DO nd=1,nbdirs
! momentum/empirical
      fzerod(nd) = cphi*kpd(nd)/lambda_r - (-(sphi*ad(nd)))/(1-a)**2
    END DO
    fzero = sphi/(1-a) - cphi/lambda_r*(1-kp)
  ELSE
    DO nd=1,nbdirs
! propeller brake region
      fzerod(nd) = cphi*kpd(nd)/lambda_r - sphi*kd(nd)
    END DO
    fzero = sphi*(1-k) - cphi/lambda_r*(1-kp)
  END IF
  print *, fzerod
