...
rotor.root = OptimizeRotorSE(naero, nstr) # RotorSE(naero, nstr)
...
# rotor.root.list_connections()

# === blade grid ===
rotor['rotor.initial_aero_grid'] = initial_aero_grid  # (Array): initial aerodynamic grid on unit radius
rotor['rotor.initial_str_grid'] = initial_str_grid  # (Array): initial structural grid on unit radius
rotor['rotor.idx_cylinder_aero'] = 3  # (Int): first idx in r_aero_unit of non-cylindrical section, constant twist inboard of here
rotor['rotor.idx_cylinder_str'] = 14  # (Int): first idx in r_str_unit of non-cylindrical section
rotor['rotor.hubFraction'] = 0.025  # (Float): hub location as fraction of radius
# ------------------

# === blade geometry ===
rotor['rotor.r_aero'] = np.array([0.02222276, 0.06666667, 0.11111057, 0.2, 0.23333333, 0.3, 0.36666667, 0.43333333,
...
rotor['rotor.precurve_sub'] = np.array([0.0, 0.0, 0.0])  # (Array, m): precurve at control points.  defined at same locations at chord, starting at 2nd control point (root must be zero precurve)
# rotor['rotor.delta_precurve_sub'] = np.array([0.0, 0.0, 0.0])  # (Array, m): adjustment to precurve to account for curvature from loading
rotor['rotor.sparT'] = np.array([0.05, 0.047754, 0.045376, 0.031085, 0.0061398])  # (Array, m): spar cap thickness parameters
rotor['rotor.teT'] = np.array([0.1, 0.09569, 0.06569, 0.02569, 0.00569])  # (Array, m): trailing-edge thickness parameters
rotor['rotor.bladeLength'] = 61.5  # (Float, m): blade length (if not precurved or swept) otherwise length of blade before curvature
# rotor['rotor.delta_bladeLength'] = 0.0  # (Float, m): adjustment to blade length to account for curvature from loading
rotor['rotor.precone'] = 2.5  # (Float, deg): precone angle
rotor['rotor.tilt'] = 5.0  # (Float, deg): shaft tilt
rotor['rotor.yaw'] = 0.0  # (Float, deg): yaw error
rotor['rotor.nBlades'] = 3  # (Int): number of blades
# ------------------
...
# === atmosphere ===
rotor['rotor.rho'] = 1.225  # (Float, kg/m**3): density of air
rotor['rotor.mu'] = 1.81206e-5  # (Float, kg/m/s): dynamic viscosity of air
rotor['rotor.shearExp'] = 0.2  # (Float): shear exponent
rotor['rotor.hubHt'] = np.array([90.0])  # (Float, m): hub height
rotor['rotor.turbine_class'] = 'I'  # (Enum): IEC turbine class
rotor['rotor.turbulence_class'] = 'B'  # (Enum): IEC turbulence class class
rotor['rotor.cdf_reference_height_wind_speed'] = np.array([90.0])  # (Float): reference hub height for IEC wind speed (used in CDF calculation)
rotor['rotor.g'] = 9.81  # (Float, m/s**2): acceleration of gravity
# ----------------------

# === control ===
rotor['rotor.control:Vin'] = 3.0  # (Float, m/s): cut-in wind speed
rotor['rotor.control:Vout'] = 25.0  # (Float, m/s): cut-out wind speed
rotor['rotor.control:ratedPower'] = 5e6  # (Float, W): rated power
rotor['rotor.control:minOmega'] = 0.0  # (Float, rpm): minimum allowed rotor rotation speed
rotor['rotor.control:maxOmega'] = 12.0  # (Float, rpm): maximum allowed rotor rotation speed
rotor['control:tsr'] = 7.55  # (Float): tip-speed ratio in Region 2 (should be optimized externally)
rotor['rotor.control:pitch'] = 0.0  # (Float, deg): pitch angle in region 2 (and region 3 for fixed pitch machines)
rotor['rotor.pitch_extreme'] = 0.0  # (Float, deg): worst-case pitch at survival wind condition
rotor['rotor.azimuth_extreme'] = 0.0  # (Float, deg): worst-case azimuth at survival wind condition
rotor['rotor.VfactorPC'] = 0.7  # (Float): fraction of rated speed at which the deflection is assumed to representative throughout the power curve calculation
# ----------------------

# === aero and structural analysis options ===
rotor['rotor.nSector'] = 4  # (Int): number of sectors to divide rotor face into in computing thrust and power
rotor['rotor.npts_coarse_power_curve'] = 20  # (Int): number of points to evaluate aero analysis at
rotor['rotor.npts_spline_power_curve'] = 200  # (Int): number of points to use in fitting spline to power curve
rotor['rotor.AEP_loss_factor'] = 1.0  # (Float): availability and other losses (soiling, array, etc.)
rotor['rotor.drivetrainType'] = 'geared'  # (Enum)
rotor['rotor.nF'] = 5  # (Int): number of natural frequencies to compute
rotor['rotor.dynamic_amplication_tip_deflection'] = 1.35  # (Float): a dynamic amplification factor to adjust the static deflection calculation
# ----------------------
...
ncomp = len(rotor['rotor.initial_str_grid'])
upper = [0]*ncomp
lower = [0]*ncomp
webs = [0]*ncomp
profile = [0]*ncomp

rotor['rotor.leLoc'] = np.array([0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.498, 0.497, 0.465, 0.447, 0.43, 0.411,
...
rotor['rotor.sector_idx_strain_spar'] = [2]*ncomp  # (Array): index of sector for spar (PreComp definition of sector)
rotor['rotor.sector_idx_strain_te'] = [3]*ncomp  # (Array): index of sector for trailing-edge (PreComp definition of sector)
...
rotor['rotor.chord_str_ref'] = np.array([3.2612, 3.3100915356, 3.32587052924, 3.34159388653, 3.35823798667, 3.37384375335,
...
rotor['rotor.materials'] = np.array(materials)  # (List): list of all Orthotropic2DMaterial objects used in defining the geometry
rotor['rotor.upperCS'] = np.array(upper)  # (List): list of CompositeSection objections defining the properties for upper surface
rotor['rotor.lowerCS'] = np.array(lower)  # (List): list of CompositeSection objections defining the properties for lower surface
rotor['rotor.websCS'] = np.array(webs)  # (List): list of CompositeSection objections defining the properties for shear webs
rotor['rotor.profile'] = np.array(profile)  # (List): airfoil shape at each radial position
# --------------------------------------


# === fatigue ===
rotor['rotor.rstar_damage'] = np.array([0.000, 0.022, 0.067, 0.111, 0.167, 0.233, 0.300, 0.367, 0.433, 0.500,
    0.567, 0.633, 0.700, 0.767, 0.833, 0.889, 0.933, 0.978])  # (Array): nondimensional radial locations of damage equivalent moments
rotor['rotor.Mxb_damage'] = 1e3*np.array([2.3743E+003, 2.0834E+003, 1.8108E+003, 1.5705E+003, 1.3104E+003,
...
rotor['rotor.Myb_damage'] = 1e3*np.array([2.7732E+003, 2.8155E+003, 2.6004E+003, 2.3933E+003, 2.1371E+003,
...
rotor['rotor.strain_ult_spar'] = 1.0e-2  # (Float): ultimate strain in spar cap
rotor['rotor.strain_ult_te'] = 2500*1e-6 * 2   # (Float): uptimate strain in trailing-edge panels, note that I am putting a factor of two for the damage part only.
rotor['rotor.eta_damage'] = 1.35*1.3*1.0  # (Float): safety factor for fatigue
rotor['rotor.m_damage'] = 10.0  # (Float): slope of S-N curve for fatigue analysis
rotor['rotor.N_damage'] = 365*24*3600*20.0  # (Float): number of cycles used in fatigue analysis  TODO: make function of rotation speed
# ----------------

# TEST NEW INPUTS
# rotor['rotor.chord_sub'] = np.array([ 1.3 ,        3.14850006 , 2.71236245 , 2.057331  ])
# rotor['rotor.control:tsr'] = 8.34767939089
# rotor['rotor.r_max_chord'] = 0.374932900033
# rotor['rotor.theta_sub'] = np.array([ 11.38729938  , 3.98744815  , 2.85248822 ,  1.91070877])
